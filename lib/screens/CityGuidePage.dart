// lib/screens/CityGuidePage.dart
...
import 'package:smart_tourism_app/models/site_category.dart'; // For SiteCategory data in TouristSite
...
class _CityGuidePageState extends State<CityGuidePage> {
  ...
  @override
  void dispose() {
    mapController?.dispose(); // Dispose the controller when done
    super.dispose();
  }
  ...
  Future<void> _fetchTouristSites() async {
    ...
    try {
      ...
      // Consider adding city filtering if needed for a specific city guide (e.g., city: 'دمشق')
      final PaginatedResponse<TouristSite> response = await tourismRepo.getTouristSites(page: 1, city: null);

      setState(() {
        // Filter out sites without valid lat/lng to avoid map errors
        ...
        // Animate camera to fit all markers if sites are loaded
        ...
        } else if (_touristSites.isEmpty && mapController != null) {
          // If no sites, go back to initial wide view
          mapController!.animateCamera(CameraUpdate.newCameraPosition(
            const CameraPosition(target: _initialCameraPosition, zoom: 7),
          ));
        }
      });
      ...
      _markers = _touristSites.map((site) {
        return Marker(
          ...
          infoWindow: InfoWindow(
            ...
            onTap: () {
              // TODO: Navigate to Tourist Site Details Page (PlaceDetailsPage)
              if (mounted) {
                 ScaffoldMessenger.of(context).showSnackBar(
                   SnackBar(content: Text('عرض تفاصيل ${site.name}')),
                 );
                 // Example Navigation:
                 // Navigator.push(context, MaterialPageRoute(builder: (_) => PlaceDetailsPage(siteId: site.id)));
              }
            },
          ),
          onTap: () {
  ...
  LatLngBounds _boundsFromLatLngList(List<LatLng> list) {
    ...
    // Add a check to prevent nulls if list is empty, though _touristSites.isNotEmpty handles this
    if (x0 == null || x1 == null || y0 == null || y1 == null) {
      return LatLngBounds(northeast: _initialCameraPosition, southwest: _initialCameraPosition);
    }
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            Expanded(
              flex: 3,
              child: _isLoading && _touristSites.isEmpty // Only show spinner if no data yet for list
                  ? const Center(child: CircularProgressIndicator(color: kPrimaryColor))
                  : _errorMessage != null
                          ...
                          ? Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'تأكد من اتصالك بالإنترنت أو أضف مواقع جديدة.',
                                    textAlign: TextAlign.center,
